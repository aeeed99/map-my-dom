...
function masterParser(str) {
    ...
    for (var i = 0; i < tagsArr.length; i++) {
        var error = !!Array.isArray(tagsArr[i]);
        ...
        console.log("TAGS BODY LINEs", tagBodyLines);
        //remove spaces used inside quotes.
        tagInfo = tagInfo.replace(/(")(\\"|\\\\|[^"\\]*)(")/g, function (match, p1, p2, p3) {
            return /*p1 +*/ p2.split(" ")[0] /*+ p3*/; //removing p1 and p3 essentially takes out quotes when rendering
                                                       // (just pure attribute shown)
        });
...
function lineify(string, max, _n) {
    string = string.replace(/(\s)\s+/g, function (match, p1, p2) {
        return p1
    }).replace(/\s$/, "");
    var count = _n || 1;
    while (string[0] === " ") {
...
//    if(useParser) arr = masterParser(arr); --to be removed; treeify should only be called with arrays of Tags already returned from masterParser
    ...
    if (arr.every(function (tag) {
            return !tag.children.legth
        })) {
